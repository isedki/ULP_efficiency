...
from logparser.Logram.Common import regexGenerator
...
def doubleMatch(tokens, indexList, doubleDictionaryList, doubleThreshold, length):
    ...
    for i in range(len(indexList)):
        ...
        if index == 0:
            ...
            if (doubleTmp1 in doubleDictionaryList and doubleDictionaryList[doubleTmp1] >= doubleThreshold) or (
                    doubleTmp2 in doubleDictionaryList and doubleDictionaryList[doubleTmp2] >= doubleThreshold):
                pass;
            else:
                dynamicIndex.append(index);
        else:
...
def tokenMatch(allTokensList, doubleDictionaryList, triDictionaryList, doubleThreshold, triThreshold, outdir,
               log_file_basename, allMessageList):
    ...
    for tokens in allTokensList:
        ...
        structured_log_lines.append((index + 1, allMessageList[index], template_id, logEvent))
