...
class LogParser:
    ...
    def __init__(self, log_format, indir='./', outdir='./result/', rex=[]):
        ...
        ----------
            ...
            savePath : the output path stores the file containing structured logs2
        """
        self.path = indir
        self.indir = indir
        self.outdir = outdir
    ...
    def getDynamicVars2(self,petit_group) :
        ...
        petit_group['event_label'] = petit_group['event_label'].map(lambda x:' '.join(dict.fromkeys(x.split())))
        petit_group['event_label'] = petit_group['event_label'].map(lambda x:' '.join(filter(None, (word.strip(punctuation) for word in x.split()))))
    ...
    def remove_word_with_special(self,sentence):
        ...
        for word in  sentence.split():
            if not any(ch.isdigit() for ch in word) and not any(not c.isalnum() for c in word)  and len(word)>1 :
                finale += word
    ...
    def parse(self,logname):
        ...
        self.tokenize()
        print
        self.df_log['EventId'] = self.df_log['event_label'].map(lambda x:self.remove_word_with_special(str(x)))
        ...
        print("++++++++++++++++parsing done in" , elapsed_timeBig)
        return  0
