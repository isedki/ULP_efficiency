...
from logparser.SLCT import regexmatch
import re
import os
import sys
import pandas as pd
...
class LogParser:
    def __init__(self, log_format, indir='../logs2/', outdir='./results/', split_threshold=4, rex=[], seed=1):
    ...
    #extract the templates according to the logs2 in each group
    def extracting(self):
    ...
    # save to logs2 in groups into different template txt
...
#calculate the distance betweent each two logs2 and save into a matrix       
def calDistance(wordLL,v,path):
    print('calculate distance between every two logs2...')
    logNum=len(wordLL)
    distList=[]
...
#the edit distance of two logs2
