...
#datasets = [ 'HDFS', 'Hadoop', 'Spark', 'Zookeeper', 'OpenStack', 'BGL', 'HPC', 'Thunderbird', 'Windows', 'Linux','Mac'] 
datasets = ['Spark', 'HDFS',   'OpenStack', 'BGL', 'HPC', 
          'Mac', 'Android', 'HealthApp', 'OpenSSH']
#datasets = ['Mac', 'Android', 'HealthApp', 'OpenSSH']
...
def is_abstract(x, y):
    ...
    x = re.sub(r'<\*>:<\*>', '<*>', x)
    y = re.sub(r'<\*>:<\*>', '<*>', y)
    number_of_stars= len(re.findall('\*', y))
    y2=transform(y)
    x2=transform(x)
    if y2 is np.NaN:
    ...
    y2= list(y2.split(" "))
    y2 = [x for x in y2 if x != '']

    
    print("x2",x2)
    print("y2",y2)
    partial = set(x2).issubset(set(y2)) and number_of_stars>0
    if m:
        print("true")
        return True , False
    else:
...
def is_similar(x, y):  
    x1= re.sub('[^A-Za-z0-9]+ ', ' ', x)  
    y1= re.sub('[^A-Za-z0-9]+ ', ' ', y) 
    doc1 = nlp(x1)
    doc2 = nlp(y1)
...
def is_abstract2(x, y):
    ...
    #m = re.match(get_pattern_from_template(x), y)
    m = re.match(transform2(x),transform2(y))
    print("x",transform2(x))
    #print("get_pattern_from_template(x)",get_pattern_from_template(x))
    print("y",transform2(y))
    if transform2(x)==transform2(y) :
        print("yeaaaaah")
        return True
    else:
        return False
